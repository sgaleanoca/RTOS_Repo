...
ntc_sensor_ctx_t* ntc_sensor_create(void) {
    ntc_sensor_ctx_t *ctx = (ntc_sensor_ctx_t*)calloc(1, sizeof(ntc_sensor_ctx_t));
    if (!ctx) return NULL;
    ...
    ESP_ERROR_CHECK(adc_oneshot_config_channel(ctx->adc_handle, NTC_PIN, &config));
    adc_calibration_init(ADC_UNIT, ADC_ATTEN_DB_12, &ctx->cali_handle);
...
ntc_data_t ntc_read_temperature(ntc_sensor_ctx_t* ctx) {
    ...
    if (adc_oneshot_read(ctx->adc_handle, NTC_PIN, &raw_adc_value) != ESP_OK) {
